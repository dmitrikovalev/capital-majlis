...
// Mint a time-limited link to the (private) attachment. Returns null on any
// failure so a missing/broken file never blocks the notification.
async function signAttachment(path: string | null): Promise<string | null> {
  if (!path) return null;
  if (!SUPABASE_URL || !SERVICE_ROLE_KEY) {
    console.error("Cannot sign attachment: missing SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY");
    return null;
  }
...
Deno.serve(async (req) => {
  try {
    ...
    const record: PartnershipRecord = payload.record;

    const signedUrl = await signAttachment(record.attachment_path);
    const attachmentRow = signedUrl
      ? `<tr><td><b>Attachment</b></td><td><a href="${escapeHtml(signedUrl)}">View file</a> <i>(link expires in 7 days)</i></td></tr>`
      : `<tr><td><b>Attachment</b></td><td>—</td></tr>`;

    // Preserve line breaks from the free-text proposal in the email body.
    ...
    const html = `
      ...
      <table cellpadding="6" style="border-collapse:collapse">
        ...
        <tr><td><b>Phone</b></td><td>${record.phone ? escapeHtml(record.phone) : "—"}</td></tr>
        <tr><td><b>Company</b></td><td>${record.company ? escapeHtml(record.company) : "—"}</td></tr>
        <tr><td><b>Subject</b></td><td>${escapeHtml(record.subject)}</td></tr>
        <tr><td valign="top"><b>Proposal</b></td><td>${messageHtml}</td></tr>
        ${attachmentRow}
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: `New partnership inquiry — ${escapeHtml(record.subject)}`,
        html,
      }),
    });
